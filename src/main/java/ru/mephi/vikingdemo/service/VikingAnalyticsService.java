...
public class VikingAnalyticsService {
    ...
    public VikingAnalyticsService(VikingStorage vikingStorage) { this.vikingStorage = vikingStorage; }
    ...
    public long countByBeardAndHair(String beardStyle, String hairColor) {
        return vikingStorage.findAll().stream()
                .filter(v -> Objects.toString(v.beardStyle()).equalsIgnoreCase(beardStyle)
                        && Objects.toString(v.hairColor()).equalsIgnoreCase(hairColor)).count();
    }

    public long countWithAxes(int axeCountTarget) {
        return vikingStorage.findAll().stream().filter(v -> v.equipment().stream()
                .filter(item -> item.name().toLowerCase().contains("axe")).count() == axeCountTarget).count();
    }

    public Optional<Viking> getRandomTallViking() {
        return vikingStorage.findAll().stream().filter(v -> v.heightCm() > 180).findAny();
    }

    public List<Viking> getLegendaryVikings() {
        return vikingStorage.findAll().stream().filter(v -> v.equipment().stream()
                .anyMatch(item -> "LEGENDARY".equalsIgnoreCase(item.quality()))).toList();
    }
    ...
    public List<Integer> getEvenIds(Integer[] ids) { return Arrays.stream(ids).filter(id -> id % 2 == 0).toList(); }
}