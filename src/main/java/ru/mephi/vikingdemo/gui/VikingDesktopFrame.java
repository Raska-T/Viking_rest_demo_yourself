...
public class VikingDesktopFrame extends JFrame {
    ...
    public VikingDesktopFrame(VikingService service, VikingStorage vikingStorage, VikingTableModel tableModel) {
        ...
        String[] titles = {
                ...
                "Рыжие с длинной бородой", "> 180 см", "Ровно 1 или 2 топора", "Тест массива ID", "Борода + Волосы"
        };
        ...
        Runnable[] actions = {
                ...
                () -> { analyticsOutputArea.setText("рыжие викинги с бородой"); analyticsService.getSortedRedBeardVikings().forEach(v -> analyticsOutputArea.append("- " + v.name() + " | Возраст: " + v.age() + " | Борода: " + v.beardStyle() + "\n")); },
                () -> analyticsService.getRandomTallViking().ifPresentOrElse(v -> analyticsOutputArea.setText("викинг выше 180 см:\n" + v.name() + " (" + v.heightCm() + " см)"), () -> analyticsOutputArea.setText("В базе нет никого выше 180 см.")),
                () -> analyticsOutputArea.setText("Количество  с ровно 1 или 2 топорами: " + analyticsService.countWithAxes()),
                () -> { Integer[] testIds = {10, 25, 7, 42, 18, 33}; analyticsOutputArea.setText("Входной массив Integer ID: [10, 25, 7, 42, 18, 33]\n");analyticsService.getMaxId(testIds).ifPresent(max -> analyticsOutputArea.append("Последняя запись (max ID): " + max + "\n")); analyticsOutputArea.append("Все четные ID: " + Arrays.toString(analyticsService.getEvenIds(testIds))); },
                () -> {String beardStyle = JOptionPane.showInputDialog(null, "Введите стиль бороды:\nSHORT, LONG, BRAIDED, FORKED, CLEAN_SHAVEN");
                    if (beardStyle == null || beardStyle.isBlank()) return;
                    String hairColor = JOptionPane.showInputDialog(null, "Введите цвет волос:\nBlond, Red, Brown, Black, Gray");
                    if (hairColor == null || hairColor.isBlank()) return;
                    long count = analyticsService.countByBeardAndHair(beardStyle, hairColor);analyticsOutputArea.setText("Количество викингов с бородой " + beardStyle + "и волосами " + hairColor + ": " + count);
                }
        };
